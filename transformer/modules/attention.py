...
class SelfAttentionClifford(nn.Module):
    def __init__(self, num_feat, num_nodes, algebra, num_heads=8):
    ...
    def forward(self, feature_matrix, attention_mask):
        bs = feature_matrix.size(0) // self.num_nodes
        ...
        q = rearrange(self.q_linear(feature_matrix), '(bs n) (h d) c -> bs h n (d c)', bs=bs,
                      n=self.num_nodes, h=self.num_heads, d=self.num_feat)
        ...
        attention_output = attention_output.transpose(1, 2).contiguous().view(bs*(self.num_nodes), self.num_heads * self.num_feat, 8)
